...
def lc_ins_dvl_sim(
    ...
    # Filter-aware-loss aux buffers (capture_aux=True; joint mode only).
    ...
    # High-rate IMU aggregation (fv3): accumulate per inter-DVL interval, reset per epoch.
        ...
        if (time - time_last_dvl) >= dvl_config['epoch_interval']:
            ...
            if dnn_mode == 'joint':
                ...
                if capture_aux:
                    ...
                    C_b_n_snapshot  = est_C_b_to_n.copy()
                correction = None
                ...
                if correction is None:
                    ...
                    if correction.shape[0] >= 6:
                        ...
                        est_C_b_to_n, est_v_eb_n, est_L_b, est_lambda_b, est_h_b, est_imu_bias, P_matrix = lc_ekf_epoch_joint(
                            dvl_v_eb_b, correction[0:3], dnn_sd, tor_s, est_C_b_to_n, est_v_eb_n, est_L_b, est_lambda_b, est_h_b,
                ...
                if collect_data:
                    ...
                    if correct_attitude:
                        att_err = -ctm_to_euler(est_C_b_to_n @ true_C_b_to_n.T)
                        label = np.concatenate([label, att_err])
                ...
                # --- DNN: apply velocity compensation (inference mode) — velocity-only ---
                if compensator is not None:
                    correction = compensator.update(innovation, v_ekf_pre, euler_pre,
                                                    meas_f_ib_b, meas_omega_ib_b,
                                                    imu_agg=imu_agg)
                ...
                if collect_data:
                    ...
                    dvl_labels_list.append(label)
                    if capture_aux:
                        # Sequential filter/drift targets (post-DVL state).
                        aux_v_pre_list.append(est_v_eb_n.copy())
                        aux_v_true_list.append(true_v_eb_n.copy())
                        aux_C_b_n_list.append(est_C_b_to_n.copy())
